...
function numeroMasGrande(arrayOfNums) {
	...
	let result = -Infinity;
	for (let i = 0; i < arrayOfNums.length; i++) {
		if (arrayOfNums[i] > result) result = arrayOfNums[i];
	}
	return result;
}
...
function mesesDelAño(array) {
	...
	// Tu código:
	let meses = [];
	let requiredMonths = ['Enero', 'Marzo', 'Noviembre'];
	for (let i = 0; i < array.length; i++) {
		let m = array[i];
		if (requiredMonths.includes(m)) meses.push(m);
	}
...
function mayorACien(array) {
	...
	let mayorACienArr = [];
	for (let i = 0; i < array.length; i++) {
		if (array[i] > 100) mayorACienArr.push(array[i]);
	}
	return mayorACienArr;
}
